// Copyright 2013, 2023 Tamás Gulácsi. All rights reserved.
//
...
func (fun Function) PlsqlBlock(checkName string) (plsql, callFun string) {
	...
	if err != nil {
		slog.Error("error preparing", "function", fun, "error", err)
		panic(fmt.Errorf("%s: %w", fun.FullName(), err))
	}
	fn := fun.AliasedName()
	...
	if i < 0 {
		slog.Error("not found", "name", fun.RealName(), "in", call)
	}
...
func demap(plsql, callFun string) (string, string) {
	...
	for _, v := range plusIdxs {
		...
		if old == "" {
			...
			slog.Error("cannot find", "idx", idx, "prev", prev)
		} else {
...
func (fun Function) prepareCall() (decls, pre []string, call string, post []string, convIn, convOut []string, err error) {
	...
	for _, arg := range fun.Args {
		arg.Attribute.Name = replHidden(arg.Attribute.Name)
		args = append(args, arg)
	}
	if fun.Returns != nil {
		args = append(args, Argument{Direction: DirOut, Attribute: *fun.Returns})
	}
	...
	for _, arg := range args {
		...
		if arg.Attribute.Type.IsScalar() {
			...
			// Collection
			// like simple, but for the arg.CollectionOf
			typ = getTableType(arg.Attribute.Type.CollectionOf.Scalar.DataType)
			...
			decls = append(decls, vn+" "+arg.Type.Name+"; --B="+arg.Name)
			if arg.IsInput() {
				pre = append(pre,
					vn+".DELETE;",
					"i1 := "+arg.Name+".FIRST;",
...
func mkRecTypName(s string) string { return s + "_Rt" }
